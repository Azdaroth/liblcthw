...
static char *test_operations()
{
  ...
  mu_assert(map != NULL, "Failed to make a map.");
  mu_assert(make_random(map), "Didn't make a random fake raddix map.");
  ...
  while(map->end > 0) {
    ...
    mu_assert(check_order(map), "RadixMap didn't stay order after delete.");
  }
