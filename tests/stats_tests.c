...
char *test_recreate()
{
  ...
  mu_assert(EQ(st->sumsq, expect.sumsq, 3), "sumsq not equal");
  mu_assert(EQ(st->sum, expect.sum, 3), "sum not equal");
  mu_assert(EQ(st->min, expect.min, 3), "min not equal");
  mu_assert(EQ(st->max, expect.max, 3), "max not equal");
  mu_assert(EQ(st->n, expect.n, 3), "max not equal");
