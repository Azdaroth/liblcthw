...
/**
* Simple Bob Jenkin's hash algorithm from the 
...
Hashmap *Hashmap_create(Hashmap_compare compare, Hashmap_hash hash, int *buckets_number)
{
  ...
  if (buckets_number != NULL) {
    amount = *buckets_number;
  } else {
    amount = DEFAULT_NUMBER_OF_BUCKETS;
  }
  map->compare = compare == NULL ? default_compare : compare;
  map->hash = hash == NULL ? default_hash : hash;
  map->buckets = DArray_create(sizeof(DArray *), amount);
...
int Hashmap_set(Hashmap *map, void *key, void *data)
{
  ...
  assign_node_to_bucket(bucket, node);
