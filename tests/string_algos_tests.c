...
char *test_find_and_scan()
{
  ...
  mu_assert(scan_i > 0, "should find another ALPHA after the first.");
...
char *test_binstr_performance()
{
  ...
  debug("BINSTR COUNT: %lu, END TIMEL %d, OPS: %f",
...
char *test_find_performance()
{
  ...
  // const_bstring to_split = (bstring)"I have ALPHA beta ALPHA and oranges ALPHA";
  // struct bstrList *split = bsplit(to_split,  " ");

  // printf("%s\n", "AAA");

  // printf("%d\n", split->qty);

  // int random = rand() % (split->qty);
