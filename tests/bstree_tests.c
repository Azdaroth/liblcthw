...
struct tagbstring test1 = bsStatic("tests data 1");
struct tagbstring test2 = bsStatic("tests data 2");
struct tagbstring test3 = bsStatic("tests data 3");
...
char *test_get_set()
{
  ...
  mu_assert(result == &expect1, "Wring value for test1.");

  rc = BSTree_set(map, &test2, &expect2);
  mu_assert(rc == 0, "Failed to set &test2");
  result = BSTree_get(map, &test2);
  mu_assert(result == &expect2, "Wring value for test2.");

  rc = BSTree_set(map, &test3, &expect3);
  mu_assert(rc == 0, "Failed to set &test3");
  result = BSTree_get(map, &test3);
  mu_assert(result == &expect3, "Wring value for test3.");
...
char *test_fuzzing()
{
  ...
  for (i = 0; i < 100; i++) {
    ...
    mu_assert(BSTree_delete(store, numbers[i]) == NULL, "Should got nothing");
